...
class SessionsTabs(Widget):
    ...
    def render_session_label(self, session: SessionDetails) -> Content:
        ...
        project_basename = Path(session.path).name if session.path else ""
        suffix = f" [italic dim]{project_basename}[/]" if project_basename else ""
        match session.state:
            case "asking":
                return Content.from_markup(
                    "[not dim $text-secondary]❯ [/]"
                    + session.title
                    + suffix
                )
            case "busy":
                return Content.from_markup(f"⌛ {session.title}{suffix}")
        return Content.from_markup(f"{session.title}{suffix}")
