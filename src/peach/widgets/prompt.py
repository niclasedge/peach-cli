...
from peach.widgets.highlighted_textarea import HighlightedTextArea
from peach.widgets.condensed_path import CondensedPath
from peach.widgets.path_search import PathSearch
from peach.widgets.plan import Plan
from peach.widgets.question import Ask, Question
from peach.widgets.slash_complete import SlashComplete
from peach.messages import UserInputSubmitted
...
from peach.path_complete import PathComplete
...
"""
    ...
    def update_suggestion(self) -> None:
        ...
        if self.cursor_at_end_of_text and "\n" not in self.text:
    ...
    def action_multiline_submit(self) -> None:
        if not self.agent_ready:
            ...
            return
        self.post_message(UserInputSubmitted(self.text, False))
        self.clear()
    ...
    def action_cursor_up(self, select: bool = False):
        if self.selection.is_empty and not select:
            ...
            if row == 0:
                self.post_message(messages.HistoryMove(-1, False, self.text))
                return
...
class Prompt(containers.VerticalGroup):
    ...
    def watch_modes(self, modes: dict[str, Mode] | None) -> None:
        from peach.visuals.columns import Columns
